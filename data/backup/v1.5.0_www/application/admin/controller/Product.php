...
class Product extends Base
{
    ...
    {
        /*所有商品没有任何参数时，默认跳转到新商城产品管理入口*/
        if (!is_language() && 1 == getUsersConfigData('shop.shop_open')) {
            $product_attr_count = Db::name('product_attr')->count();
            if (empty($product_attr_count)) {
                $this->redirect(url('ShopProduct/index'));
            }
        }
        /*end*/
    ...
    public function add()
    {
        ...
        if (!is_language() && 1 == getUsersConfigData('shop.shop_open')) {
            ...
            if (empty($product_attribute_list)) {
                $this->redirect(url('ShopProduct/add', ['typeid'=>$typeid]));
            }
        }
        /*end*/
    ...
    public function edit()
    {
        ...
        if (!is_language() && 1 == getUsersConfigData('shop.shop_open')) {
            $product_attribute_list = model('ProductAttribute')->getListByTypeid($typeid);
            if (empty($product_attribute_list)) {
                $this->redirect(url('ShopProduct/edit', ['id'=>$id]));
            }
        }
        /*end*/
    ...
    public function ajax_get_attr_input($typeid = '', $aid = '')
    {
        ...
        if (empty($str)) {
            if (!is_language() && 1 == getUsersConfigData('shop.shop_open')) {
                $str = '<div style="font-size: 12px;text-align: center;">提示：原产品参数功能停止维护，推荐使用商城中心，若有需要请先保存，然后【<a href="'.url('ShopProduct/edit', array('id'=>$aid)).'">点击这里</a>】进行更多操作。</div>';
            } else {
